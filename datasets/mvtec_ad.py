...
class MVTecAD(VisionDataset):
    ...
    def __init__(self,
                 ...
                 pin_memory=False,
                 return_mask=False,
                 include_indistr=True,
                 include_indistr_only=True,
                 return_path=False):
        ...
        if train:
            self.classes, self.class_to_idx = self._find_classes(self.subset_split, include_indistr=True, include_indistr_only=False)
        else:
            self.classes, self.class_to_idx = self._find_classes(self.subset_split, include_indistr, include_indistr_only)
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any, Any]:
        ...
        # rescale images to lie between [-1, 1] instead of [0, 1]
        image = (2 * image - 1.0)
        if self.return_mask and self.return_path:
            return image, mask, target, self.image_paths[idx]
        if self.return_path:
    ...
    def _find_classes(self, dir: str, include_indistr: bool, include_indistr_only: bool) -> Tuple[List[str], Dict[str, int]]:
        """
        ...
        if include_indistr_only:
            classes = [self.normal_str]
        elif include_indistr:
...
def get_mvtec_train_data(batch_size,  deterministic=False, size=256, root='data/mvtec-ad/',
    ...
    init_transforms = [ #transforms.RandomRotation(degrees=180),
...
def get_mvtec_train_subset_data(size=256, root='data/mvtec-ad/',
                 ...
                 return_mask=False,
                 subset="hazelnut"):
    init_transforms = transforms.Compose([ #transforms.RandomRotation(degrees=180),
                transforms.Resize(size),
                transforms.CenterCrop(size),
                #transforms.RandomHorizontalFlip(),
                transforms.ToTensor()
    ...
    target_transform = None
    mask_transform = transforms.ToTensor()
    train_dataset = MVTecAD(root=root, subset_name=subset, train=True, transform=init_transforms,
        target_transform=target_transform, mask_transform=mask_transform,
        download=download, pin_memory=pin_memory, return_mask=return_mask)
    return train_dataset


def get_mvtec_test_data(size, root='data/mvtec-ad/',
                 download=True,
                 pin_memory=False,
                 return_mask=False,
                 return_path=False):

    test_transforms = [ transforms.Resize(size),
                        transforms.CenterCrop(size),
                        transforms.ToTensor()
                        ]
    target_transform = None
...
def get_mvtec_test_subset_data(size=256, root='data/mvtec-ad/',
                 download=True,
                 pin_memory=False,
                 return_mask=False,
                 subset="hazelnut",
                 include_indistr=False,
                 include_indistr_only=False,
                 return_path = False):
    ...
    target_transform = None
    mask_transform = transforms.ToTensor()
    train_dataset = MVTecAD(root=root, subset_name=subset, train=False, transform=test_transforms,
        target_transform=target_transform, mask_transform=mask_transform,
        download=download, pin_memory=pin_memory, return_mask=return_mask,
        include_indistr=include_indistr, include_indistr_only=include_indistr_only,
